...
const Profile = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 bg-gray-50 min-h-screen">
      <div className="mb-8">
        <h2 className="text-3xl font-heading font-bold text-secondary-800">Your Profile</h2>
        <p className="text-secondary-500 mt-2">Manage your orders, notifications, and account settings</p>
      </div>
          ...
          <div className="bg-white rounded-xl shadow p-4 sticky top-4">
            <nav className="space-y-2">
              {tabs.map((tab) => (
                ...
                >
                  <tab.icon 
                    className={`w-5 h-5 mr-3 ${activeTab === tab.id ? "text-white" : "text-primary-500"}`} 
                    size={20} 
                    strokeWidth={2}
                  />
