...
const OrdersSection = () => {
  const orders = useProductStore((state) => state.getAllOrders());
            ...
            >
              ...
              >
                <div className="flex items-center space-x-4">
                  {statusIcons[order.status] && (
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${statusColors[order.status] || "bg-gray-400"}`}>
                      {React.createElement(statusIcons[order.status], { size: 20, className: "text-white" })}
                    </div>
                  )}
                  <div>
                    <h4 className="font-medium text-secondary-800">Order #{order.id}</h4>
                    <p className="text-sm text-gray-500">
                      {new Date().toLocaleDateString()} • {order.items.length} {order.items.length === 1 ? 'item' : 'items'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <span className={`px-3 py-1 text-sm font-medium text-white rounded-full ${statusColors[order.status] || "bg-gray-400"}`}>
                    {order.status}
                  </span>
              ...
              {expandedOrders[order.id] && (
                <div className="p-4">
                  {/* Order Timeline */}
                  <div className="mb-6">
                    <h5 className="text-sm font-medium text-gray-600 mb-3">Order Status</h5>
                    <div className="flex items-center">
                      {order.statusHistory.map((step, index) => (
                        <div key={index} className="flex-1 relative">
                          <div className="flex flex-col items-center">
                            <div className={`w-6 h-6 rounded-full z-10 ${statusColors[step.status] || "bg-gray-400"}`}></div>
                            <p className="text-xs text-gray-600 mt-2 text-center max-w-[80px] truncate">{step.status}</p>
                          </div>
                          {index < order.statusHistory.length - 1 && (
                            <div className={`h-1 absolute top-3 left-1/2 right-0 -mr-3 ${
                  ...
                  <div className="mb-4">
                    <h5 className="text-sm font-medium text-gray-600 mb-3">Items in this order</h5>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      <span>Subtotal</span>
                      <span>${order.items.reduce((total, item) => total + item.product.discountPrice * item.quantity, 0).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      <span>Shipping</span>
                      <span>$5.99</span>
                    ...
                    <div className="flex justify-between font-semibold text-secondary-800">
                      <span>Total</span>
                      <span>${(order.items.reduce((total, item) => total + item.product.discountPrice * item.quantity, 0) + 9.21).toFixed(2)}</span>
                    </div>
                  </div>

                  {/* Actions */}
                  <div className="mt-4 flex gap-3 justify-end">
                    <button className="px-4 py-2 border border-primary-600 text-primary-600 rounded-lg text-sm font-medium hover:bg-primary-50 transition-colors">
                      Track Order
                    </button>
